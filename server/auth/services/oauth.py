...
class OAuthService:
    ...
    def __init_oauth(self):
        ...
        if self.provider=='google' and self.provider not in self.oauth._clients:
            self.oauth.register(
                name='google',
        ...
        if self.provider=='github' and self.provider not in self.oauth._clients:
            self.oauth.register(
                name='github',
    ...
    def store_oauth_state(self, state: str):
    ...
    def verify_and_consume_oauth_state(self, state: str):
    ...
    async def oauth_login(self, request):
        ...
        client = self.oauth.create_client(self.provider)
        redirect_uri = f"http://localhost:8000/oauth/{self.provider}/callback"

        return await client.authorize_redirect(request, redirect_uri, state=state)
    
    async def oauth_callback(self, state: str, request):
        ...
        try:
            ...
            raise ValueError(f"OAuth callback error: {str(e)}")